...
class Simulation:
    def __init__(self: Simulation, n: int, ax, seed: int = 2042) -> None:
        ...
        self.leader = self.boids[0]
        
        # Ajout d'une interaction pour suivre le leader
        for boid in self.boids[1:]:
            boid.add_interaction(lambda b, pop: self.leader.x - b.x, 50)
    ...
    def plot(self: Simulation, ax) -> None:
        for boid in self.boids:
            color = "red" if boid == self.leader else ".1"
            p, *_ = ax.plot(
                *boid.x,
                color=color,
                markersize=15,
    ...
    def iteration(self: Simulation, frame: int):
        ...
        for p, boid in zip(self.artists, self.boids):
            ...
            # Mettre à jour la couleur du boid
            if boid == self.leader:
                p.set_color("red")
            else:
                p.set_color(".1")
...
simulation = Simulation(n=20, ax=ax)
